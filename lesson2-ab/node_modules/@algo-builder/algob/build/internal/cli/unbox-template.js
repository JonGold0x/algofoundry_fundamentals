 ...
 */
async function confirmDepInstallation(name, destination, isNpm) {
    const { default: enquirer } = await Promise.resolve().then(() => __importStar(require("enquirer")));
    let responses;
    try {
        responses = await enquirer.prompt([
            (0, project_creation_1.createConfirmationPrompt)("shouldInstall", `Do you want to install dapp template ${name}'s dependencies with ${(0, project_creation_1.pkgManager)(isNpm)} ?`),
        ]);
    }
    catch (e) {
        if (e === "") {
 ...
 */
async function unbox({ force, templateName, destination, isNpm, }) {
    ...
    const shouldInstallDeps = await confirmDepInstallation(templateName, normalizedDestination, isNpm);
    const packageManager = (0, project_creation_1.pkgManager)(isNpm);
    ...
    (0, project_creation_1.printSuggestedCommands)(isNpm);
