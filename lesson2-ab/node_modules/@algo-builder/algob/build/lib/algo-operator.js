...
class AlgoOperatorImpl {
     ...
     */
    async sendAndWait(rawTxns, waitRounds = web_1.types.WAIT_ROUNDS) {
     ...
     * @returns TxnReceipt which includes confirmed txn response along with txID
     */
    async waitForConfirmation(txId, waitRounds = web_1.types.WAIT_ROUNDS) {
        ...
        if (pendingInfo[confirmedRound] !== null && pendingInfo[confirmedRound] > 0) {
            return { txID: txId, ...pendingInfo };
        }
        throw new Error("timeout");
    }
    // Get receipts of group txn
    async getReceiptTxns(txns) {
        const confirmedTxInfos = await Promise.all(txns.map((txn) => {
