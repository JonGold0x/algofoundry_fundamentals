...
exports.getProgramVersion = exports.isAddedBytecblock = exports.isAddIntcblock = exports.parser = exports.assertMaxCost = exports.opcodeFromSentence = exports.wordsFromLine = void 0;
...
opCodeMap[4] = {
    ...
    "b>": opcode_list_1.ByteGreaterThan,
...
opCodeMap[6] = {
    ...opCodeMap[5],
    divw: opcode_list_1.Divw,
    bsqrt: opcode_list_1.Bsqrt,
    gloadss: opcode_list_1.Gloadss,
    acct_params_get: opcode_list_1.AcctParamsGet,
    itxn_next: opcode_list_1.ITxnNext,
    gitxn: opcode_list_1.Gitxn,
    gitxna: opcode_list_1.Gitxna,
    gitxnas: opcode_list_1.Gitxnas,
    itxnas: opcode_list_1.ITxnas,
};
/**
 * TEALv7
 */
opCodeMap[7] = {
    ...opCodeMap[6],
    base64_decode: opcode_list_1.Base64Decode,
    replace2: opcode_list_1.Replace2,
    replace3: opcode_list_1.Replace3,
    sha3_256: opcode_list_1.Sha3_256,
    ed25519verify_bare: opcode_list_1.Ed25519verify_bare,
    json_ref: opcode_list_1.Json_ref,
    block: opcode_list_1.Block,
    vrf_verify: opcode_list_1.VrfVerify,
};
/**
 * TEALv8
...
opCodeMap[8] = {
    ...opCodeMap[7],
    bn254_add: opcode_list_1.Bn254Add,
    bn254_scalar_mul: opcode_list_1.Bn254ScalarMul,
    bn254_pairing: opcode_list_1.Bn254Pairing,
    switch: opcode_list_1.Switch,
};
 ...
 */
function opcodeFromSentence(words, counter, interpreter, mode) {
    let opCode = words[0];
    ...
    if (opCodeMap[tealVersion][opCode] === undefined) {
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.UNKNOWN_OPCODE, {
            opcode: opCode,
            version: tealVersion,
            line: counter,
        });
    }
    if (mode === types_1.ExecutionMode.APPLICATION && signatureModeOps.has(opCode)) {
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.EXECUTION_MODE_NOT_VALID, {
            opcode: opCode,
            allowedIn: "signature",
            ranIn: "application",
            tealV: tealVersion,
    ...
    if (mode === types_1.ExecutionMode.SIGNATURE && applicationModeOps.has(opCode)) {
        throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.EXECUTION_MODE_NOT_VALID, {
            opcode: opCode,
...
function assertMaxCost(gas, mode, maxPooledApplCost) {
    if (mode === types_1.ExecutionMode.SIGNATURE) {
        ...
        if (gas > constants_1.LOGIC_SIG_MAX_COST) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.MAX_COST_EXCEEDED, {
                cost: gas,
                maxcost: constants_1.LOGIC_SIG_MAX_COST,
                mode: "Stateless",
            });
        }
    }
    else {
        if (gas > (maxPooledApplCost ?? constants_1.MAX_APP_PROGRAM_COST)) {
            // check max cost (for stateful)
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TEAL.MAX_COST_EXCEEDED, {
                cost: gas,
                maxcost: maxPooledApplCost ?? constants_1.MAX_APP_PROGRAM_COST,
                mode: "Stateful",
            });
        }
    }
 ...
 * @param mode Execution mode - Signature (stateless) OR Application (stateful)
 */
function assertLogicMaxLen(lsigProgramArgsSize, mode) {
    if (mode === types_1.ExecutionMode.SIGNATURE && lsigProgramArgsSize > constants_1.LogicSigMaxSize) {
...
function isAddIntcblock(ops, interpreter) {
    if (interpreter.tealVersion < 4)
        return false;
    if (ops[0] instanceof opcode_list_1.Intcblock || ops[1] instanceof opcode_list_1.Intcblock)
        return false;
    const intCount = {};
    ...
    for (const op of ops) {
        if (op instanceof opcode_list_1.Int) {
            if (intCount[op.uint64.toString()] === undefined) {
                intCount[op.uint64.toString()] = 0;
            }
            intCount[op.uint64.toString()] += 1;
            if (intCount[op.uint64.toString()] >= 2)
                return true;
        }
    }
    return false;
}
exports.isAddIntcblock = isAddIntcblock;
// check algorand is auto added byteblock for optimize size contract
function isAddedBytecblock(ops, interpreter) {
    if (interpreter.tealVersion < 4)
        return false;
    if (ops[0] instanceof opcode_list_1.Bytecblock || ops[1] instanceof opcode_list_1.Bytecblock)
        return false;
    const byteCount = {};
    ...
    for (const op of ops) {
        if (op instanceof opcode_list_1.Byte) {
            if (byteCount[op.str] === undefined) {
                byteCount[op.str] = 0;
            }
            byteCount[op.str] += 1;
            if (byteCount[op.str] >= 2)
                return true;
        }
    }
    return false;
}
exports.isAddedBytecblock = isAddedBytecblock;
/**
 *
 * @param program teal program
 * @returns pragma version of this file
 */
function getProgramVersion(program) {
    const firstLine = program.split("\n")[0];
    const ip = new interpreter_1.Interpreter();
    opcodeFromSentence(wordsFromLine(firstLine), 1, ip, types_1.ExecutionMode.APPLICATION);
