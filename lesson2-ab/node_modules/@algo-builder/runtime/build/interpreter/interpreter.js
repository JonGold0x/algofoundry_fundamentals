...
class Interpreter {
    constructor() {
        ...
        this.mode = types_1.ExecutionMode.APPLICATION;
     ...
     */
    createAccountIfAbsent(addr) {
        let account = this.runtime.ctx.state.accounts.get(addr);
        if (!account) {
            account = new index_1.AccountStore(0, { addr, sk: new Uint8Array(0) });
     ...
     */
    _getAccountFromAddr(accountPk, line, createFlag, immutable) {
        ...
        if (txAccounts?.find((buff) => (0, compare_1.compareArray)(Uint8Array.from(buff), accountPk)) !==
            undefined ||
            ...
            (this.tealVersion >= 7 &&
                immutable &&
                foreignAppIdArr?.find((foreignAppID) => (0, compare_1.compareArray)(accountPk, (0, algosdk_1.decodeAddress)((0, algosdk_1.getApplicationAddress)(foreignAppID)).publicKey)) !== undefined)) {
     ...
     */
    getAccount(accountRef, line, createFlag = false, immutable = true) {
        let account;
        let address;
        if (accountRef === 0n) {
     ...
     */
    _getAccountFromReference(accountRef, line, createFlag) {
        let address;
        ...
        if (this.runtime.ctx.tx.apfa !== undefined &&
            ...
            address = (0, algosdk_1.getApplicationAddress)(accountRef);
        }
            ...
            if (this.runtime.ctx.tx.apat) {
                address = (0, algosdk_1.encodeAddress)(this.runtime.ctx.tx.apat[Number(accIndex)]);
            }
            else {
     ...
     */
    getBudget() {
        if (this.mode === types_1.ExecutionMode.SIGNATURE)
            return constants_1.LOGIC_SIG_MAX_COST;
     ...
     *       the result is zero.
     */
    executeWithResult(program, runtime, debugStack) {
        this.runtime = runtime;
        this.program = program;
        this.instructions = (0, parser_1.parser)(program, this.mode, this);
        this.mapLabelWithIndexes();
        if (this.mode === types_1.ExecutionMode.APPLICATION) {
        ...
        if ((0, parser_1.isAddIntcblock)(this.instructions, this)) {
            this.runtime.ctx.pooledApplCost += 1;
            this.cost += 1;
        }
        if ((0, parser_1.isAddedBytecblock)(this.instructions, this)) {
            this.runtime.ctx.pooledApplCost += 1;
            this.cost += 1;
        }
        while (this.instructionIndex < this.instructions.length) {
            const instruction = this.instructions[this.instructionIndex];
            if (this.runtime.ctx.isInnerTx &&
                this.runtime.ctx.tx.type === constants_1.TransactionTypeEnum.APPLICATION_CALL &&
                this.tealVersion < constants_1.MinVersionSupportC2CCall) {
                throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.TRANSACTION.INNER_APP_CALL_INVALID_VERSION, {
            ...
            if (this.tealVersion >= 4) {
                if (this.mode === types_1.ExecutionMode.SIGNATURE) {
                    (0, parser_1.assertMaxCost)(this.cost, this.mode);
                    txReceipt.gas = this.cost;
                }
                else {
                    this.runtime.ctx.pooledApplCost += costFromExecute;
                    (0, parser_1.assertMaxCost)(this.runtime.ctx.pooledApplCost, types_1.ExecutionMode.APPLICATION, this.getBudget());
     ...
     * @returns void
     */
    assertRoundIsAvailable(round) {
        let firstAvail = this.runtime.ctx.tx.lv - constants_1.MaxTxnLife - 1;
        ...
        if (firstAvail > round || round > lastAvail || round === 0) {
            throw new runtime_errors_1.RuntimeError(errors_list_1.RUNTIME_ERRORS.GENERAL.ROUND_NOT_AVAILABLE, {
                round: round,
